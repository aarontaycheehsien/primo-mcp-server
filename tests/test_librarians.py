...
def test_example_librarian_directory_is_safe_and_usable():
    example_path = (
        Path(__file__).resolve().parents[1] / "librarian-profile.example.json"
    )

    directory, message = load_librarian_directory(example_path)

    assert message is None
    assert directory is not None
    assert len(directory.librarians) == 3
    assert len({profile.id.casefold() for profile in directory.librarians}) == 3
    assert all(
        profile.email.casefold().endswith("@example.edu")
        for profile in directory.librarians
    )
    assert all("Example" in profile.name for profile in directory.librarians)

    matches = recommend_librarians(directory, "audit fees and financial reporting")

    assert matches
    assert matches[0].librarian.id == "accounting-business"
...
def test_format_recommendations_includes_validation_instruction():
    ...
    assert (
        "Reasoning: Selected because the matched expertise terms are configured "
        "in this librarian's topic aliases and appear in the user's query. "
...
def test_format_recommendations_rejects_empty_reasoning(monkeypatch):
    matches = recommend_librarians(_directory(), "financial reporting", [])
    monkeypatch.setattr(
        "primo_mcp_server.librarians._format_match_reasoning",
        lambda match: "",
    )

    with pytest.raises(
        ValueError,
        match="requires non-empty evidence and reasoning",
    ):
        format_librarian_recommendations(matches, "financial reporting")
...
def test_format_semantic_recommendations_uses_profile_topics_not_best_for():
    ...
    assert (
        "Reasoning: Selected because the query is semantically similar to this "
        "librarian's configured profile topics. The semantic fallback met its "
        "confidence checks even though no exact keyword match was found."
    ) in output
...
def test_format_semantic_recommendations_uses_not_configured_without_topics():
    ...
    assert "Reasoning: Selected because the query is semantically similar" in output
...
def test_format_mixed_keyword_and_semantic_matches():
    ...
    assert output.count("Reasoning: Selected because") == 2
