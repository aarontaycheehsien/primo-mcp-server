...
_PROFILE_FIELD_LABELS = (
    ...
    ("keywords", "keywords"),
    ("best_for", "support areas"),
    ("schools", "school coverage"),
    ("resource_types", "resource-type coverage"),
)

_EVIDENCE_FIELD_LABELS = {
    "query": "the user's query",
    "title": "Primo result titles",
    "subjects": "Primo subject metadata",
    "keywords": "Primo keyword metadata",
    "description": "Primo result descriptions",
    "resource_type": "Primo resource types",
    "source": "Primo source metadata",
}


def _format_match_reasoning(match: LibrarianMatch) -> str:
    """Explain why a validated match was selected in user-facing terms."""
    if is_semantic_match(match):
        return (
            "Selected because the query is semantically similar to this "
            "librarian's configured profile topics. The semantic fallback "
            "met its confidence checks even though no exact keyword match "
            "was found."
        )

    matched_norms = {_normalise_text(term) for term in match.matched_terms}
    profile_fields = [
        label
        for attribute, label in _PROFILE_FIELD_LABELS
        if any(
            _normalise_text(term) in matched_norms
            for term in getattr(match.librarian, attribute)
        )
    ]
    evidence_sources = [
        _EVIDENCE_FIELD_LABELS.get(field, field.replace("_", " "))
        for field in match.evidence_fields
        if field != "semantic"
    ]

    profile_description = (
        f"this librarian's {_format_human_list(profile_fields)}"
        if profile_fields
        else "this librarian's configured profile"
    )
    evidence_description = (
        _format_human_list(evidence_sources)
        if evidence_sources
        else "the available search evidence"
    )
    return (
        f"Selected because the matched expertise terms are configured in "
        f"{profile_description} and appear in {evidence_description}. "
        "This direct overlap met the recommendation confidence threshold."
    )


class _RecommendationExplanation(NamedTuple):
    evidence: str
    reasoning: str
...
def _build_recommendation_explanation(
    ...
    """Build the required evidence-and-reasoning pair for one match."""
    if is_semantic_match(match):
        evidence = (
    ...
    reasoning = _format_match_reasoning(match)

    # A matched librarian must never be emitted without both parts of the
    # explanation contract. Failing here is preferable to returning a
    # recommendation that appears authoritative but cannot justify itself.
    if not evidence.strip() or not reasoning.strip():
        raise ValueError(
            "Librarian recommendation requires non-empty evidence and reasoning."
        )
    return _RecommendationExplanation(evidence=evidence, reasoning=reasoning)


def _format_recommendation_entry(
    index: int,
    match: LibrarianMatch,
) -> list[str]:
    """Render one librarian through the mandatory explanation template."""
    librarian = match.librarian
    semantic = is_semantic_match(match)
    explanation = _build_recommendation_explanation(match)

    lines = [
        f"{index}. Name: {_format_linked_name(librarian)}",
        f"   Title: {librarian.title or _UNCONFIGURED}",
        f"   Contact: {librarian.email or _UNCONFIGURED}",
    ]
    if semantic:
        lines.append(
    ...
    lines.extend(
        [
            f"   {_EVIDENCE_LABEL}: {explanation.evidence}",
            f"   {_REASONING_LABEL}: {explanation.reasoning}",
        ]
    )
    return lines
...
def format_librarian_recommendations(
    ...
    *,
    ...
    for i, match in enumerate(matches, start=1):
        lines.extend(_format_recommendation_entry(i, match))
